var admin = {
    ...
    set_end_date : function(){
        elem = document.getElementById('edit-field-hours-und-0-value-datepicker-popup-0');
        var open_time = document.getElementById('edit-field-hours-und-0-value-timeEntry-popup-1').value;
        var close_time = document.getElementById('edit-field-hours-und-0-value2-timeEntry-popup-1').value;
        var split_date = elem.value.split('/');
        var split_open_time = open_time.split(':');
        var split_close_time = close_time.split(':');
        var split_open_hour = split_open_time[1].substring(0,2);
        var split_close_hour = split_close_time[1].substring(0,2);
        var open_am = split_open_time[1].substring(2,4);
        var close_am = split_close_time[1].substring(2,4);
        var open_hour = (open_am == 'am') ? split_open_time[0] : split_open_time[0] + 12;
        if(close_am == 'am'){
            if(split_close_time[0] == 12){
                close_hour = 0;
            } else{
                close_hour = split_close_time[0];
            }
        } else{
            close_hour = split_close_time[0] + 12;
        }
        var open_date = new Date(split_date[2], split_date[0]-1, split_date[1], open_hour, split_open_time[1].substring(0,2));
        var close_date = new Date(split_date[2], split_date[0]-1, split_date[1], close_hour, split_close_time[1].substring(0,2));

        var open_date_U = open_date.getTime();
        var close_date_U = close_date.getTime();

        if(close_date_U < open_date_U){
            document.getElementById('edit-field-hours-und-0-value2-datepicker-popup-0').value = split_date[0]+'/'+(Number(split_date[1])+1)+'/'+split_date[2];
        } else{
            document.getElementById('edit-field-hours-und-0-value2-datepicker-popup-0').value = elem.value;
        }
    },
    set_title : function(){
